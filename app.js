...
export function sidebar(active){
 ...
 return `<aside class="sidebar"><div class="side-logo">Effortless<span>Earn</span></div><div class="sub">PUBLISHER PANEL</div><div class="walletbox"><small>Wallet Balance</small><h1>₹${money(profile?.balance||0)}</h1><span class="idbadge">${roleLabel()}: ${(user?.uid||'').slice(0,8).toUpperCase()}</span></div><div class="menu">${menu.map(m=>`<a class="${active===m[3]?'active':''}" href="${m[0]}"><span>${m[1]}</span>${m[2]}<b>›</b></a>`).join('')}<a href="#" onclick="goTelegram()"><span>✈</span>Join Telegram<b>›</b></a><a class="logout" href="#" onclick="logoutUser()"><span>↪</span>Logout<b>›</b></a></div></aside>`;
}
