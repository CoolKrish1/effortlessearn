...
export function siteBase(){ return location.origin + location.pathname.replace(/[^/]*$/,''); }
...
window.openOffer=(id)=>{ const d=campaigns.find(x=>x.id===id); if(!d)return; const link=`${location.origin}${location.pathname.replace(/[^/]*$/,'')}campaign.html?id=${d.id}&pub=${user?.uid||''}`; modal(`<button type="button" class="close" onclick="closeModal()">×</button><div style="display:flex;gap:16px;align-items:center">${imgTag(d,"preview")}<div><span class="tag">${safe(d.category||'CPA')}</span><h1>${safe(d.title)}</h1><b class="greenText">Total Potential: ₹${money(totalMax(d))}</b></div></div><hr><div class="notice terms-box"><b>Terms & Conditions</b><p>${safe(d.terms||'Read the terms carefully before starting.').replaceAll('\n','<br>')}</p></div><div class="linkbox"><b>Smart Tracking Link</b><div class="toolbar"><input class="field" value="${link}" readonly><button type="button" class="btn blue" onclick="navigator.clipboard.writeText('${link}');toast('Link copied')">Copy</button></div></div>${goalsTable(d)}<div class="notice steps-box"><b>Steps</b><p>${safe(d.instructions||d.steps||'Complete the required steps correctly.').replaceAll('\n','<br>')}</p></div><button type="button" class="btn green full" onclick="window.closeModal();return navigate('smart','smartlinks.html?campaign=${d.id}')">Create Smart Campaign</button>`); };
...
window.exportReportsCSV=()=>{
  ...
  const rows=[['Time','Offer','Event','Payout','Status','P1/UPI','P2','IP','Source','Sub1','Sub2']];
  arr.forEach(r=>rows.push([compactDate(r.time||r.createdAt),r.campaignTitle||r.offerName||r.campaignId||'',r.goalName||r.event||'BASE',r.reward||r.payout||0,reportStatus(r),r.upi||r.p1||r.worker||'',r.p2||r.refer||'',r.ip||r.ipAddress||'',r.source||'campaign',r.sub1||'',r.sub2||'']));
...
function trackUrl(l){
  const slug=encodeURIComponent(l.slug||l.id);
  const pubId=encodeURIComponent(l.publisherId||user?.uid||'');
  // Universal tracker: always use track.html so it works on Vercel, GitHub Pages, Firebase Hosting, and local preview.
  return smartBase()+'track.html?slug='+slug+(pubId?'&pub='+pubId:'');
}
...
function smartCard(l){
  ...
  return `<div class="smart-card pro-smart" id="smart_${l.id}">
    ...
    <div class="smart-links-grid">
      ...
      <div class="smart-link yellowSoft"><b>2. Tracking Status Link</b><div class="toolbar"><input class="field" value="${statusLink}" readonly><button type="button" class="btn gold" onclick="navigator.clipboard.writeText('${statusLink}');toast('Tracking link copied')">Copy</button></div><small>Open to check task status by UPI/Bank</small></div>
    </div>
  </div>`
}
...
function renderPostbackPage(){
  ...
  const htmlUrl = siteBase()+'postback.html?click_id={click_id}&event={event_id}&payout={payout}';
  shell('postback',`<div class="panel page-title"><div><h2>Postback Tracking</h2><p class="muted">Use this for auto approval and wallet credit.</p></div><button type="button" class="btn blue" onclick="savePostbackUrl()">Save</button></div><div class="grid2"><div class="panel"><h2>Recommended Cloud Function URL</h2><p class="muted">Paste this in PathakCPA Global Postback after deploying functions.</p><div class="toolbar"><input id="postbackUrl" class="field" value="${safe(cloudUrl)}"><button type="button" class="btn light" onclick="navigator.clipboard.writeText(document.getElementById('postbackUrl').value);toast('Copied')">Copy</button></div><div class="notice small"><b>Format</b><p>?click_id={click_id}&event={event_id}&payout={payout}</p></div></div><div class="panel"><h2>Testing URL</h2><p class="muted">This opens in browser only. Real network postback needs Cloud Function.</p><div class="toolbar"><input class="field" value="${safe(htmlUrl)}" readonly><button type="button" class="btn light" onclick="navigator.clipboard.writeText('${safe(htmlUrl)}');toast('Copied')">Copy</button></div><div class="macro-grid"><span>{click_id}</span><span>{event_id}</span><span>{payout}</span><span>{sub1}</span><span>{sub2}</span><span>{status}</span></div></div></div>`)
}
window.savePostbackUrl=async()=>{try{await setDoc(doc(db,'settings','main'),{postbackUrl:document.getElementById('postbackUrl')?.value||''},{merge:true});toast('Saved');await loadData();}catch(e){toast(e.message||'Save failed')}};
