...
export let user=null, profile=null, campaigns=[], submissions=[], withdraws=[], users=[], smartLinks=[], revenueShare=[];
export const $=id=>document.getElementById(id);
const baseUrl=()=>location.origin+location.pathname.replace(/[^/]*$/,'');
const safe=v=>(v??'').toString().replace(/[&<>"]/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[m]));

window.addEventListener('click',e=>{const b=e.target.closest('button'); if(b && !b.type) b.type='button';});
...
export function sidebar(active){
 const menu=[["dashboard.html","🏠","Dashboard","dashboard"],["offers.html","🎯","All Offers","offers"],["smartlinks.html","🔗","Smart Links","smart"],["reports.html","📊","Reports","reports"],["wallet.html","💳","Wallet","wallet"],["postback.html","⚙️","Postback","postback"],["profile.html","👤","Profile","profile"],["support.html","💬","Support","support"],["admin.html","🛠️","Admin","admin"]];
 return `<aside class="sidebar"><div class="side-logo">Effortless<span>Earn</span></div><div class="sub">Publisher Panel</div><div class="walletbox"><small>Wallet Balance</small><h1>₹${profile?.balance||0}</h1><span class="idbadge">${safe(user?.email||'Guest')}</span></div><div class="menu">${menu.map(m=>`<a class="${active===m[3]?'active':''}" href="${m[0]}"><span>${m[1]}</span>${m[2]}</a>`).join('')}<a href="javascript:void(0)" onclick="logoutUser()"><span>↪</span>Logout</a></div></aside>`
}
export function shell(active,content){$('root').innerHTML=`<div class="app">${sidebar(active)}<main class="main"><div class="topbar"><div><h1>${active[0].toUpperCase()+active.slice(1)}</h1><p>Auto synced with Firebase</p></div><b>${safe(profile?.role||'user')}</b></div>${content}</main></div>`}
