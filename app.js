...
export function shell(active,content){
  ...
  $('root').innerHTML=`<div class="app">${sidebar(active)}<main class="main"><div class="topbar"><div><h1>${active[0].toUpperCase()+active.slice(1)}</h1><p>Auto synced with Firebase</p></div><b>${safe(profile?.role||'user')}</b></div>${content}</main></div>`;
